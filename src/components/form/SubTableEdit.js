...
class SubTableEdit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    const { table, parentSetId, record } = props;
    const { fields } = table;
    this.state = constructDateFocusProperties(this.state, fields);
    // this.state.record = constructRecordProperties(fields);
    // this.state.record.setName = parentSetId; // eslint-disable-line react/destructuring-assignment
  ...
  handleSubmit(event) {
    ...
    const { record } = this.state;
    dispatch(subTableDataActions.update(record, `${parentTableURL}-mapping`));
    closeModal();
    loadData();
  }
...
const connectedSubTableEdit = connect(mapStateToProps)(SubTableEdit);
export { connectedSubTableEdit as SubTableEdit };
