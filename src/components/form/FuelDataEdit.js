...
class FuelDataEdit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      constants: {
  ...
  render() {
    ...
    const indexValues = cleanGrid.map(row => row[2].value);
    const weightValues = cleanGrid.map(row => row[3].value);
...
const connectedFuelDataEdit = connect(mapStateToProps)(FuelDataEdit);
export { connectedFuelDataEdit as FuelDataEdit };
